...
class InputFnc (object):
	...
	def input_function(data_dir, mode, patch_size, batch_size, buffer_size, valid_id,
		...
		with tf.name_scope('input'):
			...
			filenames = InputFnc.get_filenames(data_dir, mode, valid_id, pred_id, overlap_step, patch_size)
			...
			if mode == 'train':
				dataset = dataset.map(InputFnc.decode_train, num_parallel_calls=num_parallel_calls)
				dataset = dataset.map(InputFnc.crop_image, num_parallel_calls=num_parallel_calls)
			elif mode == 'valid':
				dataset = dataset.map(InputFnc.decode_valid, num_parallel_calls=num_parallel_calls)
			elif mode == 'pred':
				dataset = dataset.map(InputFnc.decode_pred, num_parallel_calls=num_parallel_calls)

			dataset = dataset.map(InputFnc.normalize_image, num_parallel_calls=num_parallel_calls)
